...
def register_routes(app):
    ...
    @auth_required()
    def profile():
        rooms = Room.query.filter_by(HostUserID=current_user.id).order_by(Room.RoomCreated.desc()).all()
        rooms_data = []
        for room in rooms:
            room_info = {
                'RoomID': room.RoomID,
    ...
    def room(roomid):
        ...
        if room.RoomStatus == "inactive":
            winning_restaurant = Restaurant.query.get(room.WinningRestaurant) if room.WinningRestaurant else None
            votes = Vote.query.filter_by(RoomID=roomid).all()
            restaurants = {r.id: r.name for r in Restaurant.query.filter_by(RoomID=roomid).all()}
            user_votes = {}
            for vote in votes:
            ...
            return render_template("results.html", room=room, winning_restaurant=winning_restaurant, user_votes=user_votes, restaurant_names=list(restaurants.values()))
        ...
        if not guest_user or guest_user.RoomID != roomid:
            ...
            flash("There was an issue with your session. Please enter your name again.", "warning")
            return response

        voted_restaurant_ids = [v.RestaurantID for v in Vote.query.filter_by(GuestUserID=guest_user.id)]
        restaurants_to_vote = Restaurant.query.filter(Restaurant.RoomID == roomid, ~Restaurant.id.in_(voted_restaurant_ids)).all()

        return render_template("room.html", room=room, restaurant_data=restaurants_to_vote, current_guest_user=guest_user)
    ...
    def create_new_room():
        ...
        if not restaurant_list:
            flash(f"Could not find any restaurants for '{location}'. Please try a different location.", "danger")
    ...
    def add_guest_user():
        ...
        response.set_cookie(f'guest_user_id_{room_id}', new_user.id, max_age=7*24*60*60)
        return response
    ...
    def get_room_users():
        ...
        users_data = [{"Username": u.Username, "done": u.done, "id": u.id} for u in users]
    ...
    def finalize_room():
        ...
        if not votes:
            ...
            vote_counts = {}
            for vote in votes:
                vote_counts[vote.RestaurantID] = vote_counts.get(vote.RestaurantID, 0) + vote.VoteChoice
